...
class EvenementsController extends AbstractController
{
    ...
    #[Route('/showbyidevenements/{id}', name: 'showbyidevenements')]
    public function showbyidevenements($id, EvenementsRepository $EvenementsRepo): Response
    {

        $x = $EvenementsRepo->find($id);
    
        if (!$x) {
            throw $this->createNotFoundException("evenements not found for id {$id}");
        }
    
        return $this->render('evenements/showbyidevenements.html.twig', [
            'Evenements' => $x
        ]);
    }
